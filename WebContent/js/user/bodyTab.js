...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
		...
		//$("#navBar_inner").html(_this.navBar(dataStr)).height($(window).height()-210);
		$("#navBar_inner").html(_this.navBar(dataStr)).height($(window).height()-143);
//		$("#navBar_inner").html(_this.navBar(dataStr)).height($(window).height()-270);
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				element.tabAdd(tabFilter, {
			        title : title,
			        content :"<iframe src='"+_this.attr("data-url")+"' data-id='"+tabIdIndex+"' scrolling=\"yes\" style=\"height:"+rightContentHeight+"px\"></frame>",
			        id : new Date().getTime()
			    })
				//当前窗口内容
				var curmenu = {
